...
不能把多条 MCP 流水线变成真正并行。允许多个客户端调用排队反而会引入两类故障：

1. `Future.result(timeout)` 超时不等于 COM 调用被取消。旧实现中，排队任务即使已向
   客户端报超时，稍后仍会执行副作用；客户端重试会造成重复图、重复参考线或重复导出。
2. 活动窗口是 Origin 全局状态。`win -a X` 与随后的 `layer.*` 读写若被另一条流水线
   插入，前一条调用会静默作用于错误窗口，不一定报错。

服务端现在采用以下语义：

- MCP 有状态工具实行**单飞**。已有调用在途时，新调用立即返回
  `{"ok": false, "busy": true, "not_executed": true, ...}`，不进入 COM 队列。
- worker 在执行前用 `Future.set_running_or_notify_cancel()` 认领任务。仍在排队的任务
  超时后会被取消，worker 随后直接跳过，保证没有迟到副作用。
- COM 任务一旦开始就无法安全中断；内部超时后继续等待真实结果，不把“仍在执行”
  误报成失败，也不触发自动重连重试。

外层 MCP 传输仍可能先于长 COM 调用超时。此时结果属于“未知”，正确恢复顺序是：

1. 不强杀 Origin，不立刻重放有副作用的调用；
2. 重连 `Origin.ApplicationSI`（仍是同一实例）；
3. 调 `pages_list`，必要时再读目标图/表状态；
4. 以实际存在的窗口、曲线、参考线和输出文件为准，只补建缺失项；
5. 最后再保存工程，并保留现有空工程覆盖保护。

`busy=true` 与超时不同：它明确表示调用**没有执行**，等待前一调用完成后可以安全地
重新发起；连接关闭或外层超时则必须先枚举核对，不能假设失败。
